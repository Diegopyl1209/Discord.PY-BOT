...
class Config(commands.Cog):
    ...
    async def prefix(self, ctx, *, prefix="py."):
        ...
        await ctx.send(
            f"El prefijo del servidor se ha establecido en `{prefix}`. Use `{prefix}prefix [prefix]` para cambiarlo de nuevo!"
        )
    ...
    async def deleteprefix(self, ctx):
        ...
        await ctx.send("Este prefijo del servidor se ha vuelto a establecer en el valor predeterminado.")
    ...
    async def blacklist(self, ctx, user: discord.Member):
        ...
        await ctx.send(f"Hey, he incluido en la lista negra {user.name} por ti.")
    ...
    async def unblacklist(self, ctx, user: discord.Member):
        ...
        utils.json_loader.write_json(data, "blacklist")
        await ctx.send(f"Oye, no he incluido en la lista negra {user.name} por ti.")
    ...
    async def reload(self, ctx, cog=None):
        if not cog:
            ...
            async with ctx.typing():
                embed = discord.Embed(
                    title="Recargando todos los comandos!",
                    color=0x808080,

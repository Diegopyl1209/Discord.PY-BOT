...
class Usage(commands.Cog):
    ...
    @commands.command(
        name="commandstats",
        description="Muestra un uso general de cada comando.!"
    )
    ...
    async def command_stats(self, ctx):
        ...
        for i in range(0, len(sorted_list), cmd_per_page):
            message = "Nombre del comando: `Usage % | Número de ejecuciones de comando`\n\n"
        ...
        await Pag(title="Estadísticas de uso de comandos!", color=0xC9B4F4, entries=pages, length=1).start(ctx)
